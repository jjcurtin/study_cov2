...
generate_cc <- function(n_obs, n_covs, b_xy, b_cx, b_cy, r_cc, e_x, e_cov,
                        empirical) {
  
  # generates continuous x, y, and covariates with mean = 0, variance = 1
  # reflects causal relationship where x causes y (as long as b_xy is non-zero)
  ...
  # b_xy = path coefficients between x and y
  ...
  r_cc_y <- b_cy +
    (b_cx * b_xy) +
    (cov_matrix_zero %*% b_cy) +
    ((cov_matrix_zero %*% b_cx) * b_xy)
...
generate_distr <- function(n_obs, n_covs, b_xy, r_cc, e_x, e_cov, empirical) {
  
  # generates continuous x, y, and covs with mean = 0, variance = 1
  # introduces causal relationship where x causes y (as long as b_xy is non-zero)
...
generate_conseq <- function(n_obs, n_covs, b_xy, b_cx, r_cc, e_x, e_cov, empirical) {
  ...
  # n_covs = number of covariates
  # b_xy = path coefficients between x and y
  # b_cx = path coefficients between x and covariates (should this be a vector?)
